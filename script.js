...
function animate() {
    ...
    if (isPlaying) {
        ...
        scanners.forEach(s => {
            const subSteps = 8; // Higher = more realistic "blur", lower = better performance
            const stepDt = dt / subSteps;
            const stepMove = (0.05 * simSpeed) / subSteps;

            for (let j = 0; j < subSteps; j++) {
                // Rolling Shutter: Interpolate angle and position
                const subAngle = s.angle + (Math.PI * 2 * s.hz * s.dir) * (stepDt * j);
                const subZ = drone.position.z + (stepMove * j);
                const subX = drone.position.x; // Add wobble interpolation here if desired
                const subY = drone.position.y;
                ...
                for (let i = 0; i < NUM_POINTS; i++) {
                    ...
                    const dotNormal = (pVx * s.normal.x) + (pVy * s.normal.y) + (pVz * s.normal.z);
                    ...
                    if (Math.abs(dotNormal) < 0.04 && dotBeam > hitThreshold) {
                        ...
                        if (scannedIntensity[i] > maxIntensity) {
                            maxIntensity = scannedIntensity[i];
                        }

                        const mapX = (scenePts[i * 3] / ROOM_SIZE.x + 0.5) * 200;
                        const mapZ = (scenePts[i * 3 + 2] / ROOM_SIZE.z + 0.5) * 200;
                        ctx.fillStyle = `rgba(0, 255, 204, ${reflectivity * 0.5})`;
        ...
        let currentMax = 0;
        for (let i = 0; i < scannedIntensity.length; i++) {
            if (scannedIntensity[i] > currentMax) currentMax = scannedIntensity[i];
        }
        document.getElementById('st-heat').innerText = Math.floor(currentMax);
